...
1.  Remove all but the `Reference`, `Sampling_Date`, `Gear`, and `Sand Seatrout` columns.  Assign this dataset to a new object in your workspace.  Don't forget to use backticks to select column names that have spaces.

1. Using the new object, filter again to get only Sand Seatrout and Pinfish caught with Gear 20 (hint, filter by the `Gear` column` and don't forget ot use `==`). 
