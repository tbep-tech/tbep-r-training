...
Let's reproduce the histogram and scatterplot we created in exercise 1 using ggplot. This requires us to map the `Sampling_Date` and `Pinfish` variables to the x and y aesthetics for `geom_point`. 
...
Let's modify the scatterplot we created in exercise 3 by adding a theme, a statistical summary, and facets. We'll map the `stat_smooth` function to the Gear type and also use this variable to create a three-panel plot.  
