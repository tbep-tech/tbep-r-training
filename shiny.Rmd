...
* All input objects are defined in the `ui` object, given a name inside the `fooInput()` function (`foo` meaning generic, e.g., `plot`, `table`, etc.), and then referenced in the `server` file by `input$name` (`input$n` in this case). 
    ```{r, eval = F}
    numericInput(inputId = 'n', label = 'Sample size', value = 50)
    ```
...
Now we create a function that filters these data by species range, size name, and gear and then plots the results over time. The creation and purpose of this function is explained in the link above, but in short, we use it to filter by species size classes to plot trends over time. 
